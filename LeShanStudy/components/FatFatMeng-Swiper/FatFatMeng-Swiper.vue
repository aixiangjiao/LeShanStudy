	...
	<view>
		...
		<view :class="swiperClassName">
			<swiper class="swiper" :indicator-dots="false" :autoplay="true" :circular="true"
				:interval="3000" :duration="1000" indicator-active-color="#333" @change="change">
				<swiper-item v-for="(item,index) in SwiperImglist" :key="index" :current="index">
					<image class="Imges" :src="item.image" mode="aspectFill"></image>
					<!-- #ifdef APP-NVUE || APP-PLUS-NVUE -->
					<image class="ImagesTextbg"
						src="/static/FatFatMeng-apps/FatFatMeng-image/FatFatMeng-style/FatFatMeng-SwiperTextbg2.png"
						mode="widthFix"></image>
					<!-- #endif -->
<!-- 					<view class="title">
			...
			<view class="indicator-dots-zdy-right">
				<view class="w720">
					<view v-for="(item,index) in SwiperImglist" :key="index" class="indicator-dots-item"
...
<style lang="scss">
	...
	.indicator-dots-zdy-right {
		...
		.w720 {
			...
			flex-wrap: nowrap;
			align-items: center;
			justify-content: center; //放到右边好看些
